...
def info_update2(doc,old_info, new_info):
    excelApp = Dispatch('Excel.Application')  # 打开word应用程序
    excelApp.Visible = 0  # 后台运行,不显示
    excelApp.DisplayAlerts = 0  # 不警告
    wb =excel.Workbooks.Open(doc)
    ws= wb.Worksheet('sheet1')
    
    # doc = excelApp.Workbooks.Open(doc)
    # excelApp.Selection.Find.ClearFormatting()
    # excelApp.Selection.Find.Replacement.ClearFormatting()
    excelApp.Selection.Find.Execute(old_info, False, False, False, False, False, True, 1, True, new_info, 2)
    doc.SaveAs(r'C:\Users\yongjiangao\QA_EVR\替换结果\700-014621-000.XLS')
    doc.Close()
# for file in files:
...
if __name__ == '__main__':
    ...
    for file in os.listdir(path):
        if file.endswith(".xls"): #排除文件夹内的其它干扰文件，只获取word文件
